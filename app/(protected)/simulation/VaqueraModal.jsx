...
export default function VaqueraModal({ isOpen, onClose }) {
  ...
  const getNextAvailability = (inv) => {
    ...
    return { available: remainingMs <= 0, remainingMs };
  };
  ...
  const claimInvestment = async (inv) => {
    ...
    try {
      ...
      setError("");
      alert(`Ganancia cobrada: +${data.added_amount}. Nuevo balance: ${data.new_trc20balance}`);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-[90vw] max-w-[520px] p-4">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-lg font-semibold">{t("modal_cow_shed_title")}</h2>
        ...
        {loading ? (
          ...
          <>
            <div className="mb-3 text-sm">
              {t("cows")}: <span className="font-medium">{investments.length}</span>
            </div>

            <div className="space-y-2 max-h-[50vh] overflow-y-auto">
              {investments.length === 0 && (
                <div className="text-sm text-gray-600">{t("none_active_cows")}</div>
              )}
              {investments.map((inv) => {
                ...
                return (
                  <div key={inv.id} className="border rounded p-3 flex items-center justify-between">
                    <div>
                      <div className="text-sm font-medium">{t("cow")}</div>
